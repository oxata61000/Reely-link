...
(function () {
  ...
  function renderLink(l, i) {
    ...
    var sub = l.subtitle || '';
    var badge = (l.badge ? l.badge.split(' · ') : []).map(function (p) { return p.trim(); }).filter(Boolean).map(function (p) {
      return '<span class="p-badge' + (/complet|limit|dernier|bientôt|baisse/i.test(p) ? ' is-hot' : '') + '">' + esc(p) + '</span>';
    }).join('');
